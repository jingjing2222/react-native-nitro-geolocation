///
/// JCompatGeolocationResponse.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

#pragma once

#include <fbjni/fbjni.h>
#include "CompatGeolocationResponse.hpp"

#include "GeolocationCoordinates.hpp"
#include "JGeolocationCoordinates.hpp"
#include "JNullableDouble.hpp"
#include <NitroModules/JNull.hpp>
#include <NitroModules/Null.hpp>
#include <optional>
#include <variant>

namespace margelo::nitro::nitrogeolocation {

  using namespace facebook;

  /**
   * The C++ JNI bridge between the C++ struct "CompatGeolocationResponse" and the the Kotlin data class "CompatGeolocationResponse".
   */
  struct JCompatGeolocationResponse final: public jni::JavaClass<JCompatGeolocationResponse> {
  public:
    static auto constexpr kJavaDescriptor = "Lcom/margelo/nitro/nitrogeolocation/CompatGeolocationResponse;";

  public:
    /**
     * Convert this Java/Kotlin-based struct to the C++ struct CompatGeolocationResponse by copying all values to C++.
     */
    [[maybe_unused]]
    [[nodiscard]]
    CompatGeolocationResponse toCpp() const {
      static const auto clazz = javaClassStatic();
      static const auto fieldCoords = clazz->getField<JGeolocationCoordinates>("coords");
      jni::local_ref<JGeolocationCoordinates> coords = this->getFieldValue(fieldCoords);
      static const auto fieldTimestamp = clazz->getField<double>("timestamp");
      double timestamp = this->getFieldValue(fieldTimestamp);
      return CompatGeolocationResponse(
        coords->toCpp(),
        timestamp
      );
    }

  public:
    /**
     * Create a Java/Kotlin-based struct by copying all values from the given C++ struct to Java.
     */
    [[maybe_unused]]
    static jni::local_ref<JCompatGeolocationResponse::javaobject> fromCpp(const CompatGeolocationResponse& value) {
      using JSignature = JCompatGeolocationResponse(jni::alias_ref<JGeolocationCoordinates>, double);
      static const auto clazz = javaClassStatic();
      static const auto create = clazz->getStaticMethod<JSignature>("fromCpp");
      return create(
        clazz,
        JGeolocationCoordinates::fromCpp(value.coords),
        value.timestamp
      );
    }
  };

} // namespace margelo::nitro::nitrogeolocation
