...
open class HybridNitroGeolocationSpec_cxx {
  ...
  @inline(__always)
  public final func requestLocationSettings(success: bridge.Func_void_LocationSettingsResult, options: LocationSettingsOptions, error: bridge.std__optional_std__function_void_const_LocationError_____error______) -> bridge.Result_void_ {
    do {
      try self.__implementation.requestLocationSettings(success: { () -> (LocationSettingsResult) -> Void in
        let __wrappedFunction = bridge.wrap_Func_void_LocationSettingsResult(success)
        return { (__result: LocationSettingsResult) -> Void in
  ...
  @inline(__always)
  public final func getActiveWatches() -> bridge.Result_std__vector_ActiveWatch__ {
    do {
      let __result = try self.__implementation.getActiveWatches()
      let __resultCpp = { () -> bridge.std__vector_ActiveWatch_ in
        var __vector = bridge.create_std__vector_ActiveWatch_(__result.count)
        for __item in __result {
          __vector.push_back(__item)
        }
        return __vector
      }()
      return bridge.create_Result_std__vector_ActiveWatch__(__resultCpp)
    } catch (let __error) {
      let __exceptionPtr = __error.toCpp()
      return bridge.create_Result_std__vector_ActiveWatch__(__exceptionPtr)
    }
  }
