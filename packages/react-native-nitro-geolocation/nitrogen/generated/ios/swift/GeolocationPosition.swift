///
/// GeolocationPosition.swift
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

import NitroModules

/**
 * Represents an instance of `GeolocationPosition`, backed by a C++ struct.
 */
public typealias GeolocationPosition = margelo.nitro.nitrogeolocation.GeolocationPosition

public extension GeolocationPosition {
  private typealias bridge = margelo.nitro.nitrogeolocation.bridge.swift

  /**
   * Create a new instance of `GeolocationPosition`.
   */
  init(coords: GeolocationCoordinates, timestamp: Double) {
    self.init(coords, timestamp)
  }

  var coords: GeolocationCoordinates {
    @inline(__always)
    get {
      return self.__coords
    }
    @inline(__always)
    set {
      self.__coords = newValue
    }
  }
  
  var timestamp: Double {
    @inline(__always)
    get {
      return self.__timestamp
    }
    @inline(__always)
    set {
      self.__timestamp = newValue
    }
  }
}
